...
public class TimeflowApp extends JFrame
{
	...
	public void init() throws Exception
	{
		...
		*/	
		...
		leftHolder.add(filterControlPanel, BorderLayout.CENTER);
	...
	*/
	...
	void loadDefault(Import importer, boolean readOnly)
	{
		if (!checkSaveStatus())
			return;
        try {
        	URL url = getClass().getResource("data");
    	    load(url.getPath(), importer, readOnly);
    	    noteFileUse(url.getPath());
        } 
        catch (Exception e) {
        	showUserError("Couldn't read file.");
            System.out.println(e);
	...
	public void load(final String fileName, final Import importer, boolean readOnly)
	{
		if (!checkSaveStatus())
			return;
		try
	...
	public void readExampleDirectory(){
		String[] ex=getVisibleFiles("settings/examples");
		int n=ex.length;
		examples=new String[n][2];
		for (int i=0; i<n; i++)
